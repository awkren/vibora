            ...
            try:
                ...
                for i, idx in enumerate(range(len(file.pages))):
                    ...
                    if i + 1 >= progress_counter + progress_interval or i + 1 == num:
                        ...
                        logging.info(
                            f"Decrypted {progress_counter} of {num} pages ({progress_percent:.1f}%)"
                        )
                ...
                logging.info(
                    f"Fize size after decryption: {os.path.getsize(pdf_path)} bytes"
                )
                logging.info(
                    "Finished decrypting file. Elapsed time %.3f", elapsed_time
                )
            except Exception:
