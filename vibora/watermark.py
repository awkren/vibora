  ...
  try:
    ...
    for i, page in enumerate(range(len(pdf_reader.pages))):
      ...
      if i+1 >= progress_counter + progress_interval or i+1 == num:
        ...
        logging.info(f"Added watermark to {progress_counter} of {num} pages ({progress_percent:.1f}%)")
    with open('watermarked.pdf', 'wb') as out:
      pdf_writer.write(out)
    end_time = time.time()
    elapsed_time = end_time - start_time
    logging.info(f"File size after adding watermark: {os.path.getsize('watermarked.pdf')} bytes")
    logging.info("Finished adding watermark to file. Elapsed time %.3f", elapsed_time)
