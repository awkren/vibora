...
def compress_pdf(
    ...
    try:
        ...
        start_time = (
            time.monotonic()
        )  # time.monotonic was used to avoid issues with system time changes
        ...
        if output == None:
            ...
            output_file = (
                output + ".pdf"
            )  # if an output name is passed, it receives its name
            with open(output_file, "wb") as f:
                try:
                    writer.write(f)
                except Exception as e:
        ...
        logging.info(
            f"File size after compression: {os.path.getsize(output_file)} bytes"
        )
        ...
        logging.info(
            "Percentage variation compairing to original file: -%.2f%%",
            calc_percentage_variation,
        )
